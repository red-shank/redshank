...
export type Rules<
  TValue extends FieldValues = FieldValues,
  TName extends FieldPath<TValue> = FieldPath<TValue>
> = Omit<
  RegisterOptions<TValue>,
  'valueAsNumber' | 'valueAsDate' | 'setValueAs' | 'disabled' | 'validate'
> & {
  validate?:
    | InternalValidate<FieldPathValue<TValue, TName>, TValue>
    | Record<string, InternalValidate<FieldPathValue<TValue, TName>, TValue>>;
};
