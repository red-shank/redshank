...
export const Base = ({ iconLeft, iconRight, content, onPress, withBoxShadow, withIcon = false, type = 'default', internalType = 'default', Component = onPress ? Ripple : View, style = {}, styleText = {}, }) => {
    const { colors, activeOpacity, paddingSizes, marginSizes, isDark } = useTheme();
    ...
    return (<Component onPress={onPress} disableRipple={false} disableTransform={true} activeOpacity={activeOpacity} style={styles.wrapper}>
      <Animated.View style={StyleSheet.flatten([
        ...
        {isValidChild(content) ? (content) : (<Text fontWeight="500" color={textColor} style={StyleSheet.flatten([
                styles.contentText,
