...
import { Animated, ActivityIndicator, Dimensions, View, TouchableWithoutFeedback, TouchableOpacity, StyleSheet, Text } from 'react-native';
...
export default class CardModal extends Component {
    ...
    renderTop() {
        ...
        var back = this.state.pressed ? (<TouchableOpacity style={[styles.backButton]} onPress={this._onPress}>
        ...
        return (<>
        {back}
        <Animated.Image source={this.props.image} style={[
                styles.top,
                borderStyles,
                {
    ...
    renderBottom() {
        var loading = this.state.activate == 'loading' ? (<ActivityIndicator animating={true} color="white"/>) : (<Text style={{ color: 'white', fontWeight: '800', fontSize: 18 }}>
          {this.state.activate}
        </Text>);
        var button = this.state.pressed ? (<TouchableOpacity onPress={this.activate}>
        <Animated.View style={{
        ...
        var plusButton = !this.state.activated ? (<Animated.View style={{
    ...
    render() {
        return (<View style={[styles.container, this.state.pressedStyle]}>
