...
import { getColorForBackground, getRandomColor, getTextForAvatar } from '../../utils';
export const Avatar = ({ src, text, textColor, icon, onPress, onPressIn, onPressOut, onLongPress, backgroundColor, borderColor, textStyle, showCountText = 2, innerBorderColor, bordered = !!borderColor, Component = onPress || onPressIn || onPressOut || onLongPress ? Ripple : Box, ImageComponent = ImageRN, size = 40, type = 'circle', style = {}, imageProps = {}, ...restTouchProps }) => {
    ...
    return (<Component width={size} height={size} borderRadius={borderRadiusElement} bg={innerBorderColorResolve} style={StyleSheet.flatten([
            styles.container,
            bordered && {
                borderWidth: 2,
                borderStyle: 'solid',
    ...
    // @ts-ignore
    onPress={onPress} onPressIn={onPressIn} onPressOut={onPressOut} onLongPress={onLongPress} {...restTouchProps}>
      <Box overflow="hidden" bg={_backgroundColor} width={bordered ? size - internalPaddingImage : size} height={bordered ? size - internalPaddingImage : size} borderRadius={borderRadiusElement} justifyContent="center" alignItems="center" style={StyleSheet.flatten([
            styles.container,
            !bordered && {
                padding: internalPaddingImage
            }
        ])}>
        {isShowText ? (<Text style={StyleSheet.flatten([
            ...
            {textShow}
          </Text>) : icon ? (<Icon color={textColor || getColorForBackground(_backgroundColor)} {...(icon || {})}/>) : (<Image source={sourceImage} resizeMode="cover" width={size} height={size} {...imageProps} style={StyleSheet.flatten([
                {
