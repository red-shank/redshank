...
export const Header = ({ title, leftIcon, rightIcon, titleStyle, style, statusBarHeight, scrollOffsetY, heightDynamic = 0, titleOnScroll = title, titlePosition = defaultTitlePosition, titleOnScrollPosition = 'center', background = 'background', backgroundSticky = background, }) => {
    ...
    const { height = statusBarHeight || defaultHeight, ...restStyle } = StyleSheet.flatten([defaultHeight, style]);
    const fadeInAnimation = React.useCallback((start, end) => {
    ...
    return (<>
      ...
      <Animated.View style={StyleSheet.flatten([
        ...
        <View style={styles.content}>
          {(leftIcon || showIconSpace) && (<View style={StyleSheet.flatten([
                styles.icon,
          ...
          {typeof title === 'string' && opacityNumber === 0 ? (<Animated.Text aria-level="1" numberOfLines={1} style={[
                styles.defaultTitle,
                styles.title,
                {
                    color: colors.text,
              ...
              {title}
            </Animated.Text>) : opacityNumber === 0 ? (title) : null}

          {typeof titleOnScroll === 'string' && opacityNumber !== 0 ? (<Animated.Text aria-level="1" numberOfLines={1} style={[
                styles.defaultTitle,
                styles.title,
                {
              ...
              {titleOnScroll}
            </Animated.Text>) : opacityNumber !== 0 ? (titleOnScroll) : null}

          {(rightIcon || showIconSpace) && (<View style={StyleSheet.flatten([
                styles.icon,
