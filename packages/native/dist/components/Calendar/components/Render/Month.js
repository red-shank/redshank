...
import WeekDaysNumber from './WeekDaysNumber';
...
const getWeeksIndex = (size) => Array.from(new Array(size).keys());
const daysOfWeekIndex = Array.from(new Array(7).keys());
function Month({ withoutMarginBottom }) {
    ...
    const render = useMemo(() => {
        ...
        for (const week of weeksIndex) {
            ...
            daysOfWeekIndex.forEach((day) => {
                ...
                const isAfterMonth = (week === totalWeeks - 1 && day > lastDayOfMonth) ||
            ...
            weeks.push(<WeekDaysNumber key={`week-${week}`} id={`${startMonth.format('YYYY-MM')}-week-${week}`} days={days}/>);
        }
        return weeks;
    }, [currentMonth]);
    return (<View style={StyleSheet.flatten([!withoutMarginBottom && styles.panel])}>
      <MonthLabel />
