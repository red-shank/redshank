...
export const Checkbox = ({ onChange, label, inactiveColor = 'accents2', activeColor = 'primary', value, required = false, defaultValue = false, type = 'circle', size = 'middle', error = false, }) => {
    ...
    return (<Ripple onPress={onInternalPress} style={StyleSheet.flatten([styles.container])}>
      ...
      <View style={StyleSheet.flatten([styles.labelView, styles.flexOne])}>
        ...
        <Text style={styles.flexOne} color={error && !active ? 'error' : 'text'}>
          {label}
        </Text>
      </View>
    </Ripple>);
};
const styleAnimation = (active, value) => {
