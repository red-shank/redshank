...
export const Switch = forwardRef(({ value, defaultValue, onChange, textError, bordered, styles, type = 'rounded', borderColor = 'primary', activeColor = 'inputColor', deactiveColor = 'inputColor', thumbActiveColor = 'primary', thumbDisabledColor = 'border', error = false, size = 'middle', style = {}, icon, sx, ...restSxProps }, ref) => {
    ...
    const calcSizeToggle = height * (type === 'rounded' ? 0.75 : 0.7);
    const calcSizeIcon = height * 0.4;
    return (<Box mb={2} ref={ref} style={style} sx={{
            ...styles?.root,
            ...sx
        }} {...restSxProps}>
        <TouchableOpacity activeOpacity={activeOpacity} onPress={onToggleSwitch} style={createSxStyle(styles?.touchable, theme)}>
          <Animated.View style={createSxStyle({
            borderStyle: 'solid',
            px: 0.4,
            position: 'relative',
