...
export const Select = ({ items, placeholder, disabled, value, onChange, pickerProps, children, Icon, size = 'middle', textError, error = false, color = 'accents2', background = 'inputColor', borderInputColor = 'border', placeholderColor = 'border', ...rest }) => {
    const [isOpen, setIsOpen] = React.useState(false);
    const [isError, setError] = React.useState(false);
    const { colors, borderRadius, fontSizes, isDark, borderWidth } = useTheme();
    const onInternalChange = (_value, index) => {
    ...
    const RenderIcon = () => {
        ...
        return (<InternalIcon size={20} type="antdesign" name={isOpen ? 'up' : 'down'} color={colors[borderInputColor] || borderInputColor}/>);
    };
    return (<View style={styles.wrapper}>
      <RNPickerSelect {...rest} onOpen={() => setIsOpen(true)} onClose={() => setIsOpen(false)} value={value} children={children} disabled={disabled} 
    // Mimic touchable input on both iOS and Android
    useNativeAndroidPickerStyle={false} Icon={Platform.select({
            web: undefined,
            default: RenderIcon,
        })} placeholder={{
            label: placeholder,
        ...
        }} onValueChange={onInternalChange} items={items.map(({ value: _value, label, key, ...restItem }, index) => ({
            key: key || index,
            value: _value,
            label,
            ...restItem,
