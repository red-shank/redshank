...
export const Ripple = ({ nativeID, children, onPress, onLongPress, onLayout: onLayoutProp, disableTransform, disableRipple = false, rippleColor = 'hover', rippleOpacity = 0.3, rippleDuration = 350, rippleSize = 0, rippleContainerBorderRadius = 0, rippleCentered = false, rippleSequential = false, rippleFades = true, disabled = false, onRippleAnimation = (animation, callback) => animation.start(callback), style, sx, ...props }) => {
    const theme = useTheme();
    let isMounted = React.useRef(true);
    let scale = React.useRef(new Animated.Value(1)).current;
    let unique = React.useRef(0).current;
    ...
    const renderRipple = ({ unique: _unique, progress, locationX, locationY, R }) => {
        const rippleStyle = {
    ...
    return (<TouchableWithoutFeedback disabled={disabled} onLayout={onLayout} onPressIn={onPressIn} onPressOut={onPressOut} onPress={onPress} onLongPress={onLongPress} {...(Platform.OS !== 'web' ? { nativeID } : null)}>
      <Animated.View {...sxResult.props} hitSlop={props.hitSlop} pointerEvents="box-only" style={sxResult.style}>
        {children}
