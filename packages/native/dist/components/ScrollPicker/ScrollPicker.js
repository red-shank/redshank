...
export const ScrollPicker = memo(({ items, onIndexChange, selected, onChange, renderItemText, itemHeight = 50 }) => {
    ...
    const renderItem = useCallback(({ item, index }) => {
        ...
        return (<Animated.View style={[
            ...
            <Text style={styles.pickerItem} color={centerIndex + 1 === index ? 'text' : 'gray700'}>
    ...
    return (<View style={{ height: itemHeight * 3 }}>
        <Animated.FlatList ref={flatListRef} data={modifiedItems} renderItem={renderItem} style={{ zIndex: 2 }} showsVerticalScrollIndicator={false} snapToInterval={itemHeight} onMomentumScrollEnd={momentumScrollEnd} scrollEventThrottle={16} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: true })} getItemLayout={(_, index) => ({
            length: itemHeight,
