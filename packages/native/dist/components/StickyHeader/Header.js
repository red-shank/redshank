...
export function Header({ title: _title, icon, styles, animateInHeight, setHeightLayout, isSticky, statusBarHeight = defaultStatusHeight, titlePosition: _titlePosition = defaultTitlePosition, background: _background = {
    sticky: 'header',
    ...
    const titlePosition = getResolveValue(_titlePosition);
    ...
    const fadeInBackgroundAnimation = useCallback((value, end) => {
    ...
    const fadeInTextAnimation = useCallback((value, end) => {
        if (typeof end === 'number') {
            Animated.parallel([
    ...
    const titleBase = useMemo(() => {
        if (!title?.initial)
            return null;
        if (React.isValidElement(title?.initial))
            return title?.initial;
        return (<Animated.Text aria-level="1" numberOfLines={1} style={createSxStyle({
                color: 'text',
    ...
    const titleSticky = useMemo(() => {
        if (!title?.sticky)
            return null;
        if (React.isValidElement(title?.sticky))
            return title?.sticky;
        return (<Animated.Text aria-level="1" numberOfLines={1} style={createSxStyle({
                color: 'text',
                sx: isSticky ? styles?.titleSticky : styles?.title,
    ...
    const showIconSpace = useMemo(() => {
        if (isSticky && titlePosition?.sticky === 'center')
            return true;
        return titlePosition?.initial === 'center';
    }, [isSticky, titlePosition]);
    return (<Animated.View onLayout={(layout) => {
      ...
      <Box style={[STYLES.content]}>
        {(icon?.left || showIconSpace) && (<View style={StyleSheet.flatten([
                STYLES.icon,
        ...
        {(icon?.right || showIconSpace) && (<View style={StyleSheet.flatten([
                STYLES.icon,
